...
import { REFUNDS } from '../lib/mock';

const COLS = [
  { key:'id',       label:'Refund ID',    render:v=><span style={{fontFamily:'var(--body)',fontSize:12,color:'var(--info)',fontWeight:600}}>#{v}</span> },
  { key:'txId',     label:'Transaction',  render:v=><span style={{fontFamily:'var(--body)',fontSize:12,color:'var(--gold)'}}>#{v}</span> },
  ...
  { key:'ref',      label:'Safepay Ref',  render:v=><span style={{fontFamily:'monospace',fontSize:11,color:'var(--text3)'}}>{v}</span> },
...
function RefundModal({ onClose, onSubmit }) {
  ...
  async function submit() {
    if (!form.txId || !form.amount) { setErr('Transaction ID and amount are required.'); return; }
    ...
    onSubmit({
      ...
      ref:      `SP-REF-${Date.now().toString().slice(-5)}`,
    });
    setLoading(false);
  }
  ...
  return (
    <div style={{ position:'fixed', inset:0, background:'rgba(5,5,5,0.5)', display:'flex', alignItems:'center', justifyContent:'center', zIndex:1000 }} onClick={onClose}>
      <div style={{ background:'var(--surface)', borderRadius:14, padding:32, width:420, maxWidth:'90vw', border:'1px solid var(--border)', boxShadow:'0 20px 60px rgba(5,5,5,0.18)' }} onClick={e=>e.stopPropagation()}>
        ...
        {[
          { label:'Transaction ID',      key:'txId',   ph:'e.g. EL-98214',            type:'text'   },
          { label:'Refund Amount (PKR)', key:'amount', ph:'e.g. 12000',                type:'number' },
          { label:'Reason (optional)',   key:'reason', ph:'Customer request, damaged…', type:'text'   },
        ].map(f => (
          <div key={f.key} style={{ marginBottom:16 }}>
            ...
            <input type={f.type} placeholder={f.ph} value={form[f.key]}
              ...
              style={{ width:'100%', padding:'11px 13px', boxSizing:'border-box', background:'var(--bg)', border:'1.5px solid var(--border)', borderRadius:8, fontFamily:'var(--body)', fontSize:13, color:'var(--black)' }} />
          </div>
        ))}
        ...
        <div style={{ display:'flex', gap:10, marginTop:8 }}>
          ...
          <button onClick={submit} disabled={loading} style={{ flex:1, padding:12, background:loading?'var(--text3)':'var(--black)', border:'none', borderRadius:8, fontFamily:'var(--body)', fontSize:13, fontWeight:500, cursor:loading?'not-allowed':'pointer', color:'#F9F7F3' }}>
...
export default function Refunds() {
  ...
  function handleNew(r) {
    setRefunds(p => [r, ...p]);
    setShowModal(false);
    setSuccess(`Refund #${r.id} initiated — tagged to transaction #${r.txId}.`);
    ...
    <div style={{ padding:32 }} className="fade-up">
      <div style={{ display:'flex', justifyContent:'space-between', alignItems:'flex-start', marginBottom:24, gap:16, flexWrap:'wrap' }}>
        <div style={{ display:'flex', gap:12, flexWrap:'wrap' }}>
          <MiniStat label="Total Refunded" value={`Rs ${(total/1000).toFixed(0)}K`} />
          <MiniStat label="Refund Count"   value={refunds.length} />
          <MiniStat label="Avg Refund"     value={`Rs ${Math.round(avg/1000)}K`} />
        </div>
        <button onClick={()=>setShowModal(true)} style={{ background:'var(--black)', color:'#F9F7F3', border:'none', borderRadius:8, padding:'11px 20px', fontFamily:'var(--body)', fontSize:13, fontWeight:500, cursor:'pointer', display:'flex', alignItems:'center', gap:6, whiteSpace:'nowrap' }}>
          + New Refund
        </button>
      </div>

      {success && (
        <div style={{ background:'var(--success-bg)', border:'1px solid rgba(42,110,71,0.2)', borderRadius:8, padding:'11px 16px', marginBottom:20, fontFamily:'var(--body)', fontSize:13, color:'var(--success)' }}>
          {success}
        </div>
      )}
      ...
      <div style={{ background:'var(--info-bg)', border:'1px solid rgba(47,95,154,0.18)', borderRadius:8, padding:'11px 16px', marginBottom:20, display:'flex', alignItems:'center', gap:10 }}>
        ...
        <span style={{ fontFamily:'var(--body)', fontSize:12, color:'var(--info)' }}>
          All refunds are automatically tagged to their original transaction record in real time.
        </span>
      </div>

      <Card>
        <CardHeader title="Refund History" subtitle="Full audit log of all refund actions" />
        <DataTable columns={COLS} rows={refunds} empty="No refunds issued yet." />
      </Card>

      {showModal && <RefundModal onClose={()=>setShowModal(false)} onSubmit={handleNew} />}
    </div>
  );
}
